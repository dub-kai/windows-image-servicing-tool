...
function Update-UpdatesActionButtons {
    ...
    Set-UiEnabled -Root $script:ctx.Page -Name 'BtnUpdatesWorkflowIntegrate' -Enabled ($canSelect -and $currentCanIntegrate)
    Set-UiEnabled -Root $script:ctx.Page -Name 'BtnUpdatesWorkflowIntegrateAll' -Enabled ($canSelect -and (@($batchTargets).Count -gt 0))
    Set-UiEnabled -Root $script:ctx.Page -Name 'BtnUpdatesWorkflowPreflight' -Enabled ($canSelect -and (@($script:mountItems).Count -gt 0))
